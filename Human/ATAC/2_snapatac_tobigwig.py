...
df.write_csv(os.path.join(root_directory, f'bigwig/{groupby}_bigwig',f"{groupby}_by_peaks.csv"))
# merged_df=pd.DataFrame(list(df['Peaks'].to_pandas().str.split('\:|\-')),columns=['chrom','start','end'])
# merged_df.name='.'
# merged_df.to_csv(os.path.join(root_directory, f'bigwig/{groupby}_bigwig',"merged_peaks.bed"),sep='\t',header=None,index=False)
data.close()
