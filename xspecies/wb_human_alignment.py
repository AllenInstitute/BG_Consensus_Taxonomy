...
#     "Siletti": "/allen/programs/celltypes/workgroups/rnaseqanalysis/HMBA/Aim1_Atlases/Human/BasalNuclei/Siletti_Basal_nuclei.h5ad"
# }
...
#         adata.obs["organism_sci"] = "Homo_sapiens"
...
# adata.write_h5ad(os.path.join(work_dir, "HMBA_Human_Macaque_Marmoset_Siletti_BG_pre-alignment.h5ad"))

## ----------------------------------------
adata = ad.read_h5ad(os.path.join(work_dir, "HMBA_Human_Macaque_Marmoset_Siletti_BG_pre-alignment.h5ad"))

## ----------------------------------------
## Remove Siletti cluster from extra-BG regions
adata = adata[~adata.obs["cluster_id"].isin(["2", "3", "147", "151", "152", "203", "205", 
                                                "311", "319", "320", "336", "402", "429", "432" 
                                                "435", "436", "438", "439", "440", "441"]), :].copy()

## ----------------------------------------
for celltype in ["Nonneuron"]:
    print(f"Processing {celltype} cells...")
    if celltype == "Neuron":
        adata_trim = ~(adata.obs["celltype"].isin(["non_neuron"]) | adata.obs["Neighborhood"].isin(["Nonneuron"]))
    ...
    sc.tl.leiden(adata_hvg, flavor="igraph", n_iterations=2, resolution=2, key_added="leiden_X_scVI")
    ##
    adata_hvg.X = adata_hvg.layers["UMIs"].copy()  # Use the UMIs layer for downstream analysis
    sc.pp.normalize_total(adata_hvg, target_sum=1e6)
    ...
    adata_hvg.write(cxgdir + f"/HMBA_Human_Macaque_Marmoset_Siletti_BG_alignment_{celltype}.h5ad")

## ----------------------------------------
# adata_trim = ~adata.obs.leiden_X_scVI.isin(['15', '63', '66', '67', '69', '70', '71', '72', '73'])
# adata_trim = adata_trim & ~((adata.obs["celltype"] == "NN-IMN-GC") & adata.obs["macosko_fig1_region_label"].isin(["BS", "CB", "HPF", "HY", "Isocortex", "MY", "TH", "MB"]))
...
## ----------------------------------------
# adata_trim = ~adata.obs["celltype"].isin(["Subpallium-GABA;HY-EA-Glut-GABA", "TH-EPI-Glut"])
