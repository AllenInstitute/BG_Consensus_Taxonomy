...
# adata.write_h5ad(os.path.join(work_dir, "HMBA_Human_Macaque_Marmoset_Mouse_snRNA-seq_BG_alignment_v6.h5ad"))

## ----------------------------------------
adata = ad.read_h5ad(os.path.join(work_dir, "HMBA_Human_Macaque_Marmoset_Mouse_snRNA-seq_BG_alignment_v6.h5ad"))

## ----------------------------------------
for celltype in ["Neuron"]:
    ...
    if celltype == "Neuron":
        ...
        adata_trim = adata.obs["AIT21.nbhd"].isin(["NN-IMN-GC"]) | adata.obs["Neighborhood"].isin(["Neuron"])
        adata_trimmed = adata[adata_trim]
...
## Save model
model.save(dir_path=work_dir + "/pre-print-integration_v6/")
...
adata_hvg.write(cxgdir + "/HMBA_Human_Macaque_Marmoset_Mouse_snRNA-seq_BG_alignment_neurons.h5ad")
