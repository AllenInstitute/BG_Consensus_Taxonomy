...
##
model_path = f"/allen/programs/celltypes/workgroups/rnaseqanalysis/EvoGen/OCTO/aibs-octo-dnaseq-modeling/basal-ganglia/models/crested/{species_name}/87.keras"
...
trainer = crested.tl.Crested(
    ...
    config=config,
    project_name=f"{species_name}_basal_ganglia_finetune",  ## Main directory for the project (species)
...
## Load data and model
adata = ad.read_h5ad(os.path.join(work_dir, "data", species_name, "crested_adata", f"{run_name}_finetune_crested.h5ad"))
model = keras.models.load_model(os.path.join(model_dir, "60.keras"), compile=False)
