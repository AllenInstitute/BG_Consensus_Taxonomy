...
EXPRESSION_THRESHOLD = 2  # 2.197 seems too strict in human BG; 1 still ?!
CLUSTER_KEY = 'cluster_id'  # Key in .obs that defines cell clusters
...
nt_identity_bool_df.to_csv(
    f"{work_dir}/{species}_BG_neurotransmitter_identity_by_cluster_th_{EXPRESSION_THRESHOLD}.csv",
    index=True
)

print(nt_identity_bool_df)
print(nt_identity_bool_df.sum(axis=1).sum(axis=0))
