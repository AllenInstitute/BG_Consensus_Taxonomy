...
expressologs = read.csv(file.path(analysis_dir, anno_term, paste0(anno_term, "_expressologs_with_metrics.csv")))
...
## Save
write.csv(auroc_tau, file.path(analysis_dir, anno_term, paste0(anno_term, "_expressologs_auroc_tau_metrics.csv")), row.names = FALSE)

## Gene sets save out
conserved = auroc_tau %>% filter(auroc > 0.75) %>% select(gene) %>% distinct()
conserved_tf = conserved %>% filter(gene %in% tf_database)
conserved_markers = auroc_tau %>% filter(auroc > 0.75 & xspecies_min > 0.75) %>% select(gene) %>% distinct()

## Per species markers tau based
markers = {}
for(species in c("human", "macaque", "marmoset")) {
  gene_dir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/HMBA/Aim1_Atlases/BasalGanglia_paper_package/analysis/gene_sets"
  auroc_tau_markers = auroc_tau %>% filter(get(species) > 0.75) %>% select(gene) %>% distinct()
  print(dim(auroc_tau_markers))
  markers[[species]] = auroc_tau_markers
  write.table(auroc_tau_markers, file.path(gene_dir, paste0(species, "_marker_genes.txt")), sep="\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
}

## Write out
# gene_dir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/HMBA/Aim1_Atlases/BasalGanglia_paper_package/analysis/gene_sets"

# write.table(conserved, file.path(gene_dir, paste0("conserved_genes.txt")), sep="\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
# write.table(conserved_tf, file.path(gene_dir, paste0("conserved_TF_genes.txt")), sep="\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
# write.table(conserved_markers, file.path(gene_dir, paste0("conserved_marker_genes.txt")), sep="\t", row.names = FALSE, col.names = FALSE, quote = FALSE)

# gene_family = "/allen/programs/celltypes/workgroups/rnaseqanalysis/HMBA/Aim1_Atlases/BasalGanglia_paper_package/analysis/gene_family"
# synGO = read.xlsx(file.path(gene_family, "synGO_and_families.xlsx"), sheet=2)

# adhesion = synGO %>% filter(Category == "cell_synaptic_adhesion_molecules") %>% select(Gene_symbol) %>% distinct()
# volt_gated = synGO %>% filter(Category == "voltage_gated_channels_others") %>% select(Gene_symbol) %>% distinct()

# write.table(adhesion, file.path(gene_dir, paste0("synGO_cell_synaptic_adhesion_molecules.txt")), sep="\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
# write.table(volt_gated, file.path(gene_dir, paste0("synGO_voltage_gated_channels_others.txt")), sep="\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
##
ggsave(paste0(analysis_dir, "/figures/expressologs_auroc_", "xspecies_min", "_scatterplot_final.png"), plot = plot.me, width = 4, height = 6, dpi = 900)
