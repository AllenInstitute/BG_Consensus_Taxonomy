...
for anno_term in ["Group"]:
    ...
    for file in expr_files:
        ...
        adata.obs_names = adata.obs_names.str.replace(" ", "_", regex=True)
        ## Restrict to cell types of interest
        adata = adata[adata.obs_names.isin(celltype_set), :].copy()
        ##
    ...
    species_order = pd.Series(["human", "macaque", "marmoset", "mouse"]).tolist()
    ...
    if os.path.exists(os.path.join(wkdir, "MSN_vignette", anno_term.lower())) is False:
        os.mkdir(os.path.join(wkdir, "MSN_vignette", anno_term.lower()))
    ## Write expressologs data.frame to csv
    expressologs.to_csv(os.path.join(wkdir, "MSN_vignette", anno_term.lower(), "expressologs_MSN_with_mouse_"+anno_term.lower()+".csv"), index=False)
