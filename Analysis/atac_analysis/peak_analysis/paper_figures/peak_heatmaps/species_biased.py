...
species_biased = pd.read_csv(os.path.join(analysis_dir, "atac", "conservation", "species_biased_peaks.csv"))
species_biased = species_biased.loc[species_biased["species"] == "human"]  # Focus on human-biased peaks for now
...
for species, h5ad_path in species_h5ads.items():
    ...
    if species == "human":
        species_adata = species_adata[:, species_adata.var_names.isin(species_biased['region'])]
        species_adata = species_adata[:, species_biased['region']]
        species_adata.var["human_ID"] = species_biased["region"].values
        species_adata.var["gini_scores"] = species_biased.set_index('region').loc[species_adata.var_names, 'human_gini'].values
        species_adata.var.set_index('human_ID', inplace=True)
    else:
        species_adata = species_adata[:, species_adata.var_names.isin(species_biased[f'{species}_ID'])]
        species_adata = species_adata[:, species_biased[f'{species}_ID']]
        species_adata.var["human_ID"] = species_biased["region"].values
        species_adata.var["gini_scores"] = species_biased.set_index(f'{species}_ID').loc[species_adata.var_names, f'{species}_gini'].values
        species_adata.var.set_index('human_ID', inplace=True)
    ...
    plt.savefig(os.path.join(analysis_dir,"atac", "conservation", "figures", f"{species}_human_biased_heatmap.png"), dpi=700)
