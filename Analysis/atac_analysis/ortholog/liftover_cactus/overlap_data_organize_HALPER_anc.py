...
for species, species_peak_file in species_peak_files.items():
    ...
    save_file = os.path.join(cactus_path, f"{species}_zoonomia_overlap_HALPER_ancestral.h5ad")
    ...
    speciesTo_overlapFiles = speciesTo_tFiles = [f for f in  glob.glob(f"{path_to_species_overlaps[species]}/*HALPER_.narrowPeak.gz") if "Anc" in f]
    ...
    for f in tqdm(speciesTo_overlapFiles):
        ...
        df["ID"] = df["ID"].str.replace(r":250$", "", regex=True)
        df.set_index("ID", inplace=True)
        ## Ensure order matches 
        species_bed_df = species_bed
        species_bed_df["overlap"] = 0
        species_bed_df.loc[species_bed_df.index.intersection(df.index), "overlap"] = df.loc[species_bed_df.index.intersection(df.index),"target_length"].values
